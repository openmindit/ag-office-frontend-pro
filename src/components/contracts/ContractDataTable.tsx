...
export default function ContractDataTable({
  ...
  const allSelected =
    contracts.length > 0 &&
    contracts.every((contract) => selectedIds.has(contract.id));
  ...
  const handleToggleRow = (contractId: string) => (checked: boolean) => {
    setSelectedIds((prevSelected) => {
      const nextSelected = new Set(prevSelected);
      if (checked) {
        nextSelected.add(contractId);
      } else {
        nextSelected.delete(contractId);
      }
      return nextSelected;
    });
  };
              ...
              {contracts.map((contract) => {
                ...
                return (
                  ...
                  >
                    <TableCell className="px-4 py-4 border border-gray-100 dark:border-white/[0.05] dark:text-white/90 whitespace-nowrap">
                      <div className="flex gap-3">
                        <div className="mt-1">
                          <Checkbox
                            checked={isSelected}
                            onChange={handleToggleRow(contract.id)}
                          />
                        </div>
                        <div>
                    ...
                    <TableCell className="px-4 py-4 font-normal text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm dark:text-white/90 whitespace-nowrap">
                      <Badge size="sm" color={contract.is_active ? "success" : "error"}>
