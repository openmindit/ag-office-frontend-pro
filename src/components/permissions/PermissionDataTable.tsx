...
export default function PermissionDataTable({
  ...
  const allSelected =
    ...
    permissions.every((permission) => selectedIds.has(permission.id));

  const handleToggleAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(new Set(permissions.map((permission) => permission.id)));
      return;
    }
    setSelectedIds(new Set());
  };

  const handleToggleRow = (permissionId: string) => (checked: boolean) => {
                    ...
                    <TableCell className="px-4 py-4 border border-gray-100 dark:border-white/[0.05] dark:text-white/90 whitespace-nowrap">
                      <div className="flex gap-3">
                        <div className="mt-1">
                          <Checkbox
                            checked={isSelected}
                            onChange={handleToggleRow(permission.id)}
                          />
                        </div>
                        <div>
