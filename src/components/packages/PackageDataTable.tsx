...
interface PackageDataTableProps {
  ...
  onStatusFilterChange: (status: "draft" | "published", checked: boolean) => void;
  onResetFilters: () => void;
}

const statusColors: Record<string, "warning" | "success" | "light"> = {
  DRAFT: "warning",
  PUBLISHED: "success",
};
...
export default function PackageDataTable({
  ...
  const allSelected =
    packages.length > 0 && packages.every((pkg) => selectedIds.has(pkg.id));
  ...
  const handleToggleRow = (packageId: string) => (checked: boolean) => {
    setSelectedIds((prevSelected) => {
      const nextSelected = new Set(prevSelected);
      if (checked) {
        nextSelected.add(packageId);
      } else {
        nextSelected.delete(packageId);
      }
      return nextSelected;
    });
  };
                  ...
                  <div className="flex flex-col gap-2 text-sm text-gray-700 dark:text-gray-300">
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(event) =>
                          onStatusFilterChange("published", event.target.checked)
                        }
                      />
                      Published
                    </label>
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(event) =>
                          onStatusFilterChange("draft", event.target.checked)
                        }
                      />
                      Draft
                    </label>
          ...
          <Table>
            ...
            <TableBody>
              ...
              {packages.map((pkg) => {
                ...
                const badgeColor = statusColors[pkg.status] ?? "light";
                ...
                return (
                  ...
                  >
                    <TableCell className="px-4 py-4 border border-gray-100 dark:border-white/[0.05] dark:text-white/90 whitespace-nowrap">
                      <div className="flex gap-3">
                        <div className="mt-1">
                          <Checkbox
                            checked={isSelected}
                            onChange={handleToggleRow(pkg.id)}
                          />
                        </div>
                        <div>
                    ...
                    <TableCell className="px-4 py-4 font-normal text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm dark:text-gray-400">
                      <p className="line-clamp-2 max-w-[320px]">{pkg.description ?? "-"}</p>
                    </TableCell>
                    <TableCell className="px-4 py-4 font-normal text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm dark:text-white/90 whitespace-nowrap">
                      <div>
                        <p>{formatDate(pkg.valid_from)}</p>
                        <span className="text-sm text-gray-500 dark:text-gray-400">
                          {formatDate(pkg.valid_to)}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell className="px-4 py-4 font-normal text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm dark:text-white/90 whitespace-nowrap">
