...
export default function RoleDataTable({
  ...
  const allSelected =
    roles.length > 0 && roles.every((role) => selectedIds.has(role.id));

  const handleToggleAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(new Set(roles.map((role) => role.id)));
      return;
    }
    setSelectedIds(new Set());
  };

  const handleToggleRow = (roleId: string) => (checked: boolean) => {
